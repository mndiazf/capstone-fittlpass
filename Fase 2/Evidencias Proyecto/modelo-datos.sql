...
CREATE TABLE public.app_user (
	...
	CONSTRAINT app_user_access_status_check CHECK (((access_status)::text = ANY ((ARRAY['NO_ENROLADO'::character varying, 'ACTIVO'::character varying, 'BLOQUEADO'::character varying])::text[]))),
...
CREATE TABLE public.membership_plan (
	...
	CONSTRAINT membership_plan_period_unit_check CHECK (((period_unit)::text = ANY ((ARRAY['WEEK'::character varying, 'MONTH'::character varying, 'TOTAL'::character varying])::text[]))),
	CONSTRAINT membership_plan_pkey PRIMARY KEY (code),
	CONSTRAINT membership_plan_plan_scope_check CHECK (((plan_scope)::text = ANY ((ARRAY['ONECLUB'::character varying, 'MULTICLUB'::character varying])::text[])))
);
...
CREATE TABLE public.user_role (
	...
	CONSTRAINT user_role_role_kind_check CHECK (((role_kind)::text = ANY ((ARRAY['MEMBER'::character varying, 'STAFF'::character varying])::text[])))
);
...
CREATE TABLE public.app_user_profile (
	...
	CONSTRAINT app_user_profile_profile_id_fkey FOREIGN KEY (profile_id) REFERENCES public.user_profile(id),
...
CREATE TABLE public.app_user_role (
	...
	CONSTRAINT app_user_role_user_id_role_id_branch_id_key UNIQUE (user_id, role_id, branch_id),
	CONSTRAINT app_user_role_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branch(id),
	CONSTRAINT app_user_role_role_id_fkey FOREIGN KEY (role_id) REFERENCES public.user_role(id),
	CONSTRAINT app_user_role_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.app_user(id)
);
CREATE INDEX ix_app_user_role_role ON public.app_user_role USING btree (role_id);
CREATE INDEX ix_app_user_role_user ON public.app_user_role USING btree (user_id);
...
CREATE TABLE public.branch_status (
	...
	CONSTRAINT branch_status_status_check CHECK (((status)::text = ANY ((ARRAY['OPEN'::character varying, 'CLOSED'::character varying, 'TEMP_CLOSED'::character varying])::text[]))),
...
CREATE TABLE public.face_enrollment (
	...
	CONSTRAINT face_enrollment_status_check CHECK (((status)::text = ANY ((ARRAY['NOT_ENROLLED'::character varying, 'ENROLLED'::character varying])::text[]))),
...
-- public.role_access_template definition

-- Drop table

-- DROP TABLE public.role_access_template;
...
CREATE TABLE public.role_access_template (
	...
	CONSTRAINT role_access_template_role_id_branch_id_name_key UNIQUE (role_id, branch_id, name),
	CONSTRAINT role_access_template_branch_id_fkey FOREIGN KEY (branch_id) REFERENCES public.branch(id),
	CONSTRAINT role_access_template_role_id_fkey FOREIGN KEY (role_id) REFERENCES public.user_role(id)
);


-- public.user_access_assignment definition

-- Drop table

-- DROP TABLE public.user_access_assignment;
...
CREATE TABLE public.user_access_assignment (
	...
	CONSTRAINT user_access_assignment_user_id_template_id_valid_from_key UNIQUE (user_id, template_id, valid_from),
	CONSTRAINT user_access_assignment_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.role_access_template(id),
	CONSTRAINT user_access_assignment_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.app_user(id)
);
CREATE INDEX ix_user_access_assignment_user ON public.user_access_assignment USING btree (user_id);
...
CREATE TABLE public.user_membership (
	...
	CONSTRAINT user_membership_status_check CHECK (((status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'EXPIRED'::character varying])::text[]))),
...
CREATE TABLE public.access_log (
	...
	CONSTRAINT access_log_result_check CHECK (((result)::text = ANY ((ARRAY['GRANTED'::character varying, 'DENIED'::character varying])::text[]))),
	CONSTRAINT access_log_source_check CHECK (((source)::text = ANY ((ARRAY['FACE'::character varying, 'MANUAL'::character varying, 'BACKOFFICE'::character varying, 'TEST'::character varying])::text[]))),
...
-- public.role_access_slot definition

-- Drop table

-- DROP TABLE public.role_access_slot;
...
CREATE TABLE public.role_access_slot (
	...
	CONSTRAINT role_access_slot_day_of_week_check CHECK (((day_of_week >= 1) AND (day_of_week <= 7))),
	CONSTRAINT role_access_slot_hour_check CHECK (((hour >= 0) AND (hour <= 23))),
	CONSTRAINT role_access_slot_pkey PRIMARY KEY (id),
	CONSTRAINT role_access_slot_template_id_day_of_week_hour_key UNIQUE (template_id, day_of_week, hour),
	CONSTRAINT role_access_slot_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.role_access_template(id) ON DELETE CASCADE
);
CREATE INDEX ix_role_access_slot_template ON public.role_access_slot USING btree (template_id, day_of_week, hour);